    ...
    private void doResolveAllUncached0(final String hostname,
        ...
        if (inflightLookups != null) {
            if (additionals == null || additionals.length == 0) {
                        ...
                        public void operationComplete(Future<? super List<InetAddress>> future) {
                            if (future.isSuccess()) {
                                ...
                                if (isTimeoutError(cause)) {
                                    ...
                                    // the remote server to be overloaded for some short amount of time or because
                                    // UDP packets were dropped on the floor. In this case lets try to just do the
                                    // query explicit to not cascade this possible temporary failure.
                                    final DnsServerAddressStream nameServerAddrs =
                                            dnsServerAddressStreamProvider.nameServerAddressStream(hostname);
                                    new DnsAddressResolveContext(DnsNameResolver.this, originalPromise,
                                            hostname, additionals, nameServerAddrs,
                ...
                } else if (maxNumConsolidation == 0 || inflightLookups.size() < maxNumConsolidation) {
